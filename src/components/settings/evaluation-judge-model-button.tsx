...
import type { EvaluationJudgeModelConfig } from "app-types/self-learning";
...
export function EvaluationJudgeModelButton() {
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [selectedValue, setSelectedValue] = useState(NONE_VALUE);

  const { data: config } = useSWR<EvaluationJudgeModelConfig | null>(
    JUDGE_MODEL_KEY,
    fetcher,
  );
    ...
    .filter(
      (provider) =>
        ...
        provider.models.some(
          (model) => model.modelType === "llm" && model.enabled,
        ),
    )
    .map((provider) => ({
      ...
      models: provider.models
        .filter((model) => model.modelType === "llm" && model.enabled)
        .map((model) => ({
          uiName: model.uiName,
  ...
  async function saveConfig() {
    ...
    setSaving(true);
    try {
      const response = await fetch(JUDGE_MODEL_KEY, {
        method: "PUT",
      ...
      await swrMutate(JUDGE_MODEL_KEY);
      toast.success("Evaluation judge model configured");
      setOpen(false);
    } catch {
      toast.error("Failed to save evaluation judge model");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function clearConfig() {
    ...
    try {
      ...
      setSelectedValue(NONE_VALUE);
      await swrMutate(JUDGE_MODEL_KEY);
      toast.success("Evaluation judge model cleared");
      setOpen(false);
    } catch {
      toast.error("Failed to clear evaluation judge model");
    } finally {
      setSaving(false);
    }
  }
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-80" align="end">
        <div className="space-y-4">
          <div>
            <h4 className="font-medium text-sm">Self-Learning Judge Model</h4>
            <p className="mt-1 text-muted-foreground text-xs">
              Choose the LLM used to score conversations, explain judgments, and
              propose reusable user memories for Emma.
            </p>
          </div>
